...
def cluster_results(data, transform='normalize', n_columns=2, figsize=(5.5,3.5), fname=None):
    ...
    plots = set(plots).intersection(data.keys())
    n_plots = len(plots)
...
def cluster_pie(axis, y, colors=None):
    ...
    n_labels = ['Cluster %d \n (%s)' % (m,'{:,d}'.format(n)) 
                for m,n in enumerate(sizes,1)]
    kwargs = dict(explode=explode, labels=n_labels, autopct='%1.1f%%', 
                  shadow=True, startangle=90, colors=colors, 
                  wedgeprops = dict(edgecolor='black'))
    axis.pie(sizes, **kwargs)
    axis.axis('equal')
    
def cluster_scatter(axis, x1, x2, y, colors=None, a=0):
    ...
    for k in np.unique(y):
        ...
        kwargs = dict(c=gaussian_kde(xx)(xx), s=10, alpha=1, cmap=cmap) 
        axis.scatter(xx[0], xx[1], **kwargs)
        axis.text(X[0].mean(),X[1].mean(),str(k+1), **props)
    ...
    axis.set_xlim(tuple([np.nanpercentile(x1,i) for i in [a,100-a]]))
    axis.set_ylim(tuple([np.nanpercentile(x2,i) for i in [a,100-a]]))
    
def cluster_histogram(axis, x, y, colors=None, bins=100, sigma=1, mle=True):
    ...
    mle : bool, optional, (default:True)
    \t If True, Maximum Likelihood Estimation (mle) is 
    \t performed and located in PDF
    '''
    n_labels = len(np.unique(y))
...
class cluster_factors:
    ...
    def fit(self, X, y, factor=None, method='scatter', a=0, colors=None, 
            bins=100, sigma=1, mle=True, fname=None):
        ...
        \t 'a' refers to the probability outside the confidence 
        ...
        labels = XY['labels'].reset_index(drop=True)
        ...
        for (n,m) in enumerate(loc):
            ...
            x2 = sample[columns[n]]
            if method=='scatter':
                cluster_scatter(axis,x1,x2,labels,colors,a)
            elif method=='hist':
                cluster_histogram(axis,x2,labels,colors,bins,sigma,mle)
        fig.tight_layout()
...
class radar_plot:
    ...
    **params : dictionary of properties, optional
    ...
    def fit(self, X, y, q=50, title=None, colors=None, fname=None):
    ...
    def __plot(self, axis, X, y, q=50, colors=None):
        ...
        for n,k in enumerate(y):
            ...
            kwargs = dict(c=colors[n], label='Cluster {0}'.format(n+1))
